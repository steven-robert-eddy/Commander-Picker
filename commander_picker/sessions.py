...
The shared sessions.db connection/schema/error type live in
`store.py` -- this module re-exports `connect`/`SessionError`/
`DATA_DIR`/`SESSIONS_DB_PATH` from there so every existing call site
(`sessions.connect()`, `sessions.SessionError`, etc.) keeps working
unchanged. See `challenge.py` for the 32-deck challenge tracker and
`pods.py` for the pod tracker -- both used to live in this file too,
split out once it grew to house three unrelated feature areas.
"""
...
# DATA_DIR/SESSIONS_DB_PATH/connect aren't referenced by name below --
# re-exported so `sessions.connect()`/`sessions.SESSIONS_DB_PATH`/etc.
# keep resolving for cli.py and every other existing external caller.
from commander_picker.store import DATA_DIR, SESSIONS_DB_PATH, SessionError, connect  # noqa: F401
